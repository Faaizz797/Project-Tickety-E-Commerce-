        ...
        <div class="flex flex-wrap items-center justify-between w-full gap-8">
            <div class="w-full max-w-[400px] flex flex-col gap-4">
                ...
                <h1 class="text-[36px] md:text-[48px] text-white font-bold">
                    Empower Your
                    <span
                        class="text-dark-indigo bg-butter-yellow inline-flex items-center h-[49px] w-max">Passion</span>
                    Today
                </h1>
    ...
    <section id="eventSection" class="container relative max-w-screen-xl py-10">
        ...
        <div class="flex justify-between items-center gap-4 mb-[50px]">
            ...
            @if (!request()->has('all_events'))
                <a href="{{ request()->fullUrlWithQuery(['all_events' => 1]) }}" class="btn-primary bg-secondary">
                    View All
                </a>
            @endif
        </div>
        ...
        <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-[30px]">
            @foreach ($events as $event)
                <x-frontend.card-event :cover="$event->thumbnail" :title="$event->name" :category="$event->category->name" :isPopuler="$event->is_populer"
                    :date="$event->start_time" :price="$event->start_from" :description="$event->headline" :routes="route('detail', $event->slug)" />
            @endforeach
        </div>
    </section>
        ...
        <div class="container relative max-w-screen-xl py-10">
            ...
            <div class="flex justify-between items-center gap-4 mb-[50px]">
                ...
                @if (!request()->has('all_categories'))
                    <a href="{{ request()->fullUrlWithQuery(['all_categories' => 1]) }}" class="btn-primary bg-secondary">
                        View All
                    </a>
                @endif
            </div>
            ...
            <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-[30px] relative">
                @foreach ($categories as $category)
                    <x-frontend.card-category :icon="$category->icon ?? asset('assets/svgs/ic-movie.svg')" :name="$category->name" :totalEvents="$category->events_count" 
